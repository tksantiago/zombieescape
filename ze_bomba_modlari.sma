...
new modetext[][] = { "Normal", "Yakinlik (Proximity)", "Carpma (Impact)", "Lazer (Laser)", "Hareket (Motion)", "Kumanda (SatchelCharge)", "Takip (Homing)" }
...
{
	static menu
	settingsmenu[id] = menu_create("Modlu Bombalar - Ana Menu", "menu_handler")
	menu = settingsmenu[id]
	
	add_option_toggle(menu, OPTION_ENABLE_NADE_MODES, "Modlu Bombalari Aktif Et", "Evet", "Hayir")
	
	menu_additem(menu, "Yapilandirma Dosyasini Yurut")
	menu_additem(menu, "Genel Ayarlar", _, _, get_option(OPTION_ENABLE_NADE_MODES) ? callbacks[1] : callbacks[0])
	menu_additem(menu, "Mod Kontrol Menu", _, _, get_option(OPTION_ENABLE_NADE_MODES) ? callbacks[1] : callbacks[0])
	menu_additem(menu, "Mod Limit & Test Menu", _, _, is_nademodes_enabled() ? callbacks[1] : callbacks[0])
	menu_additem(menu, "Vurus Puani Menusu", _, _, is_nademodes_enabled() ? callbacks[1] : callbacks[0])
	menu_additem(menu, "Mod Hasar Ayarlari", _, _, is_nademodes_enabled() ? callbacks[1] : callbacks[0])
	menu_additem(menu, "Ic Islevsel Ayarlar", _, _, is_nademodes_enabled() ? callbacks[1] : callbacks[0])
...
stock cvar_menu(id, page = 0, menu_set = 0)
{
	...
	switch (menu_set)
	{
		case 1:
		{
			settingsmenu[id] = menu_create("Modlu Bombalar - Genel Ayarlar", "cvar_menu_handler")
			menu = settingsmenu[id]
			
			add_nade_option(menu, OPTION_NADES_IN_EFFECT, "Bu bombalarda mod kullanilsin:")
			add_option_quatrotoggle(menu, OPTION_RESOURCE_USE, "Plugin Efektleri", "\rKapali", "\yNormal", "\yAkilli", "\yDusuk Bant Genisligi")
			add_option_toggle(menu, OPTION_PLAY_SOUNDS, "Bomba Sesleri", "Acik", "Kapali")
			add_option_toggle(menu, OPTION_MONSTERMOD_SUPPORT, "Canavar Modu Destegi", "Acik", "Kapali")
			add_option_toggle(menu, OPTION_MSG_SVC_BAD, "SVC_BAD Hatasi Fix", "Acik^n   \yNot: \wServer'da Bir Problem Olursa Bunu Acin!^n", "Kapali^n   \yNot: \wServer'da Bir Problem Olursa Bunu Acin!^n")
			add_option_toggle(menu, OPTION_BOT_ALLOW, "Botlarla Oynarken de Aktif Et", "Evet^n", "Hayir^n") 
			
			add_option_toggle(menu, OPTION_DISPLAY_MODE_ON_DRAW, "Izleme Modunda da Aktif Et", "Evet", "Hayir")
			add_option_toggle(menu, OPTION_RESET_MODE_ON_THROW, "Bomba Atildiginda Modu Sifirla", "Evet^n", "Hayir^n")
			
			add_option_toggle(menu, OPTION_REMOVE_IF_DIES, "Oyuncu Olurse Modunu Sifirla", "Evet", "Hayir")
			add_option_toggle(menu, OPTION_TEAM_PLAY, "Takim Oyunu (Takim Arkadaslari Bombadan Etkilenmez)", "Evet^n", "Hayir^n")
			
			add_option_toggle(menu, OPTION_SUPPRESS_FITH, "Bombayi Atinca ^"Fire in the hole!^" Yazsin.", "Evet^n", "Hayir^n")
			
			add_option_toggle(menu, OPTION_UNITS_SYSTEM, "Birim Sistemi:", "Metre", "Inc")
			
		}
		case 2:
		{
			settingsmenu[id] = menu_create("Modlu Bombalar - Mod Kontrol", "cvar_menu_handler")
			menu = settingsmenu[id]
			
			add_option_toggle(menu, OPTION_NORMAL_ENABLED, "Normal Bombalari Aktif Et.", "Evet", "Hayir")
			add_option_toggle(menu, OPTION_PROXIMITY_ENABLED, "Yakinlik Sensorlu Bombayi Aktif Et.", "Evet", "Hayir")
			add_option_toggle(menu, OPTION_IMPACT_ENABLED, "Carpip Patlayan Bombayi Aktif Et.", "Evet", "Hayir")
			add_option_toggle(menu, OPTION_TRIP_ENABLED, "Bozuk Bombayi Aktif Et.", "Evet", "Hayir")
			add_option_toggle(menu, OPTION_MOTION_ENABLED, "Hareket Sensorlu Bombayi Aktif Et.", "Evet", "Hayir")
			add_option_toggle(menu, OPTION_SATCHEL_ENABLED, "Uzaktan Kumandali Bombayi Aktif Et", "Evet", "Hayir")
			add_option_toggle(menu, OPTION_HOMING_ENABLED, "Dusmani Takip Eden Bombayi Aktif Et", "Evet^n", "Hayir^n")
			
			
			add_option_toggle(menu, OPTION_PROXIMITY_LOS, "Yakinlik Sensorlu Bombayi Oyuncu Gorus Dogrultusunda Ise Patlat", "Evet", "Hayir", OPTION_PROXIMITY_ENABLED)
			add_option_toggle(menu, OPTION_MOTION_LOS, "Hareket Sensorlu Bombayi Oyuncu Gorus Dogrultusunda Ise Patlat", "Evet", "Hayir", OPTION_MOTION_ENABLED)
			add_option_toggle(menu, OPTION_SATCHEL_DELAY, "Uzaktan Kumandali Bombaya Patlama Arasindaki Sureye Ekle", "Evet", "Hayir", OPTION_SATCHEL_ENABLED)
		}
		case 3:
		{
			settingsmenu[id] = menu_create("Modlu Bombalar - Limit & Test Menusu", "cvar_menu_handler")
			menu = settingsmenu[id]
			
			add_option_tritoggle(menu, OPTION_LIMIT_SYSTEM, "Limit Sistemi", "\rKapali^n", "\yHer Bomba Icin Kullanilabilir^n", "\yNeredeyse Her Bomba Icin Kullanilabilir^n")
			
			add_cell_option(menu, OPTION_LIMIT_PROXIMITY, "Yakinlik Sensorlu Bomba Atma Limiti", "Tane", OPTION_LIMIT_SYSTEM)
			add_cell_option(menu, OPTION_LIMIT_TRIP, "Bozuk Bomba Atma Limiti", "Tane", OPTION_LIMIT_SYSTEM)
			add_cell_option(menu, OPTION_LIMIT_MOTION, "Motion throw limit", "Tane", OPTION_LIMIT_SYSTEM)
			add_cell_with_space_option(menu, OPTION_LIMIT_SATCHEL, "Uzaktan Kumandali Bomba Atma Limiti", "Tane", OPTION_LIMIT_SYSTEM)
			
			add_option_toggle(menu, OPTION_INFINITE_GRENADES, "Sonsuz Bomba", "Evet", "Hayir")
			add_option_toggle(menu, OPTION_INFINITE_FLASHES, "Sonsuz Flash", "Evet", "Hayir")
			add_option_toggle(menu, OPTION_INFINITE_SMOKES, "Sonsuz Sis", "Evet", "Hayir")
		}
		case 4:
		{
			settingsmenu[id] = menu_create("Modlu Bombalar - Vurus Puani Menusu", "cvar_menu_handler")
			menu = settingsmenu[id]
			
			add_option_tritoggle(menu, OPTION_MATERIAL_SYSTEM, "Etkilesim Sistemi", "\rKapali", "\yIkincil Patlama", "\yVurus Puanli Sistem")
			...
			switch (get_option(OPTION_MATERIAL_SYSTEM))
			{
				case 0:
				{
					add_option_toggle(menu, OPTION_HITPOINT_DEATH, "Bombadan Olum", "Patlama^n", "Parcalanma^n",OPTION_MATERIAL_SYSTEM)
					
					add_cell_option(menu, OPTION_HITPOINT_NORMAL, "Normal Bomba Puani", "Puan", OPTION_MATERIAL_SYSTEM)
					add_cell_option(menu, OPTION_HITPOINT_PROXIMITY, "Yakinlik Sensorlu Bomba Puani", "Puan", OPTION_MATERIAL_SYSTEM)
					add_cell_option(menu, OPTION_HITPOINT_IMPACT, "Carpip Patlayan Bomba Puani", "Puan", OPTION_MATERIAL_SYSTEM)
					add_cell_option(menu, OPTION_HITPOINT_TRIP, "Bozuk Bomba Puani", "Puan", OPTION_MATERIAL_SYSTEM)
					add_cell_option(menu, OPTION_HITPOINT_MOTION, "Hareket Sensorlu Bomba Puani", "Puan", OPTION_MATERIAL_SYSTEM)
					add_cell_option(menu, OPTION_HITPOINT_SATCHEL, "Uzaktan Kumandali Bomba Puani", "Puan", OPTION_MATERIAL_SYSTEM)
					add_cell_option(menu, OPTION_HITPOINT_HOMING, "Dusmani Takip Eden Puani", "Puan", OPTION_MATERIAL_SYSTEM)
				}
				
				case 1:
				{
					add_option_toggle(menu, OPTION_SEC_EXPLO_AFFECT, "Ikinci Patlama", "\yHer Bomba Her Birini Etkiler!^n", "\yHe Bombasi Hepsini Etkiler^n")
					add_float_unit_option(menu, OPTION_RADIUS_SEC_EXPLOSION, "Ikinci Patlamanin Yari Capi Etkisi", "")
				}
				...
				case 2:
				{
					...
					add_cell_option(menu, OPTION_HITPOINT_NORMAL, "Normal Bomba Puani", "Puan", OPTION_NORMAL_ENABLED)
					add_cell_option(menu, OPTION_HITPOINT_PROXIMITY, "Yakinlik Sensorlu Bomba Puani", "Puan", OPTION_PROXIMITY_ENABLED)
					add_cell_option(menu, OPTION_HITPOINT_IMPACT, "Carpip Patlayan Bomba Puani", "Puan", OPTION_IMPACT_ENABLED)
					add_cell_option(menu, OPTION_HITPOINT_TRIP, "Bozuk Bomba Puani", "Puan", OPTION_TRIP_ENABLED)
					add_cell_option(menu, OPTION_HITPOINT_MOTION, "Hareket Sensorlu Bomba Puani", "Puan", OPTION_MOTION_ENABLED)
					add_cell_option(menu, OPTION_HITPOINT_SATCHEL, "Uzaktan Kumandali Bomba Puani", "Puan", OPTION_SATCHEL_ENABLED)
					add_cell_option(menu, OPTION_HITPOINT_HOMING, "Dusmani Takip Eden Puani", "Puan^n", OPTION_HOMING_ENABLED)
					
					add_float_option(menu, OPTION_HITPOINT_FF, "Dost Atesi", "%", OPTION_MATERIAL_SYSTEM)
					add_float_option(menu, OPTION_HITPOINT_INTER_DMG, "Bombalar Arasindaki Hasar", "%", OPTION_MATERIAL_SYSTEM)
		...
		case 5:
		{
			settingsmenu[id] = menu_create("Modlu Bombalar - Hasar Ayarlari", "cvar_menu_handler")
			menu = settingsmenu[id]
			
			add_option_tritoggle(menu, OPTION_DAMAGE_SYSTEM, "Hasar Sistemi", "\rKapali^n", "\yHer Mod Icin Aktif^n", "\yYapilandirilabilir Mod^n")
			
			add_float_option(menu, OPTION_DMG_SELF, "Kendine Hasari", "%", OPTION_DAMAGE_SYSTEM)
			add_float_option(menu, OPTION_DMG_THROUGH_WALL, "Duvara Hasari", "%", OPTION_DAMAGE_SYSTEM)
			
			switch (get_option(OPTION_DAMAGE_SYSTEM))
			{
				case 0:
				{
					add_float_option(menu, OPTION_DMG_TEAMMATES, "Takim Arkadasi Hasari", "%^n", OPTION_DAMAGE_SYSTEM)
				}
				
				case 1:
				{
					add_float_option(menu, OPTION_DMG_TEAMMATES, "Takim Arkadasi Hasari", "%^n", OPTION_FRIENDLY_FIRE)
					
					add_float_option(menu, OPTION_DMG_NORMAL, "Bombanin Genel Carpma Hasari","Kez")
				}
				...
				case 2:
				{
					...
					add_float_option(menu, OPTION_DMG_NORMAL, "Normal Bomba - Carpma Hasari","Kez",OPTION_NORMAL_ENABLED)
					add_float_option(menu, OPTION_DMG_PROXIMITY, "Yakinlik Sensorlu Bomba - Carpma Hasari","Kez", OPTION_PROXIMITY_ENABLED)
					add_float_option(menu, OPTION_DMG_IMPACT, "Carpip Patlayan Bomba - Carpma Hasari","Kez", OPTION_IMPACT_ENABLED)
					add_float_option(menu, OPTION_DMG_TRIP, "Bozuk Bomba - Carpma Hasari","Kez", OPTION_TRIP_ENABLED)
					add_float_option(menu, OPTION_DMG_MOTION, "Hareket Sensorlu - Carpma Hasari","Kez", OPTION_MOTION_ENABLED)
					add_float_option(menu, OPTION_DMG_SATCHEL, "Uzaktan Kumandali Bomba - Carpma Hasari","Kez", OPTION_SATCHEL_ENABLED)
					add_float_option(menu, OPTION_DMG_HOMING, "Dusmani Takip Eden Bomba - Carpma Hasari","Kez", OPTION_HOMING_ENABLED)
				}
			}
		}
		case 6:
		{
			settingsmenu[id] = menu_create("Modlu Bombalar - Ic Islevsel Ayarlar", "cvar_menu_handler")
			menu = settingsmenu[id]
			
			add_float_option(menu, OPTION_EXPLOSION_DELAY_TIME, "Bombalarin Genel Patlama Zamani", "Saniye^n")
			
			add_float_option(menu, OPTION_ARM_TIME_PROXIMITY, "Yakinlik Sen.Bom. Kolu Suresi", "Saniye", OPTION_PROXIMITY_ENABLED)
			add_float_unit_option(menu, OPTION_RADIUS_PROXIMITY, "Yakinlik Algilama Uzunlugu", "^n", OPTION_PROXIMITY_ENABLED)
			
			add_float_option(menu, OPTION_ARM_TIME_TRIP, "Bozuk Bomba Suresi", "Saniye", OPTION_TRIP_ENABLED)
			add_float_unit_option(menu, OPTION_TRIP_DETECT_DISTANCE, "Bozuk Bomba Algilama Suresi", "", OPTION_TRIP_ENABLED)
			add_float_unit_option(menu, OPTION_TRIP_FLY_SPEED, "Bozuk Bomba Ucma Hizi", "^n", OPTION_TRIP_ENABLED)
			
			add_float_option(menu, OPTION_ARM_TIME_SATCHEL, "Uzaktan Kumandali Bomba Kolu Suresi", "Saniye^n", OPTION_SATCHEL_ENABLED)
			
			add_float_option(menu, OPTION_ARM_TIME_MOTION, "Hareket Sen.Bom Kolu Suresi", "Saniye", OPTION_MOTION_ENABLED)
			add_float_unit_option(menu, OPTION_RADIUS_MOTION, "Hareket Sensoru Algilama Uzunlugu", "^n", OPTION_MOTION_ENABLED)
			
			add_float_option(menu, OPTION_HOMING_EXTRATIME, "Takipci Bomba Ilave Patlatma Suresi", "Saniye", OPTION_HOMING_ENABLED)
			add_float_unit_option(menu, OPTION_HOMING_SCAN_RANGE, "Takipci Bomba Algilama Menzili", "", OPTION_HOMING_ENABLED)
			add_float_unit_option(menu, OPTION_HOMING_SUPER_RANGE, "Takipci Bomba Super Algilama Menzili", "", OPTION_HOMING_ENABLED)
			add_float_unit_option(menu, OPTION_HOMING_SPEED_ADD, "Takipci Bomba Havada Hizlanmasi", "", OPTION_HOMING_ENABLED)
		}
	}
...
stock add_float_unit_option(menu, Option:control_option, const basetext[], const extratext[], Option:displayif = Option:-1)
{
	...
	format(itemtext, charsmax(itemtext), "%s: \y%s \r%s%s", basetext, value, get_option(OPTION_UNITS_SYSTEM) ? "metre" : "inc", extratext)
	menu_additem(menu, itemtext, cmd, _, (displayif != Option:-1 && !get_option(displayif)) ? callbacks[0] : callbacks[1])
}
...
public changemode(id, NadeRace:NADE_TYPE)
{
	...
	switch (mode[id][NADE_TYPE])
	{
		...
		case NADE_PROXIMITY:
		{
			client_print(id, print_center, "Mode - Yakinlik (Proximity)")
		}
		
		case NADE_IMPACT:
		{
			client_print(id, print_center, "Mode - Carpma (Impact)")
		}
		
		case NADE_TRIP:
		{
			client_print(id, print_center, "Mode - Lazer (Laser)")
		}
		
		case NADE_MOTION:
		{
			client_print(id, print_center, "Mode - Hareket (Motion)")
		}
		
		case NADE_SATCHEL:
		{
			client_print(id, print_center, "Mode - Kumanda (SatchelCharge)")
		}
		
		case NADE_HOMING:
		{
			client_print(id, print_center, "Mode - Takip (Homing)")
		}
