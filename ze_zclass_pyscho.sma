...
new const zclass_name[] = "Pyscho"
new const zclass_info[] = "['G']"
new const zclass_model[] = "pc_zombi_star"
...
new const sound_smoke[] = "ze_star/zombi_smoke.wav"
new idsprites_smoke
...
public zp_user_infected_post(id)
{
	...
	if(zp_get_user_zombie_class(id) == idclass)
	{
		...
		zp_colored_print(id, "^x04[Star]^x01 Sua habilidade e^x04 Smoking^x01. Tempo^x04 %.1f ^x01seconds.", smoke_timewait)
	}
}
...
public RemoveWaitSmoke(taskid)
{
	...
	zp_colored_print(id, "^x04[Star]^x01 Sua habilidade^x04 Smoking^x01 esta pronta.")
}
