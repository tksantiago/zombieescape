...
#define MAX_ENT_ARRAY   143	/* Is for max 4096 entities (128*32=4096) ? 128 */
...
/* Server global */
new bool:g_bPreparation//,bool:g_bZpReallyRunning
...
public zp_get_team(id){
	new temp_team = _:cs_get_user_team(id)
	switch (temp_team)
	{
		case CS_TEAM_UNASSIGNED:
		{ 
			return _:CS_TEAM_UNASSIGNED
		}
		case CS_TEAM_SPECTATOR:
		{
			return _:CS_TEAM_SPECTATOR
		}
		case CS_TEAM_T:
		{
			if (zp_has_round_started())//zp_get_user_nemesis(id) || zp_get_user_zombie(id))
				return _:CS_TEAM_T;
			else
				return _:CS_TEAM_CT;
		}
...
public plugin_init()
{
	...
	set_task(1.5, "checkTeams", 0, _, _, "b")
}
...
{
        set_pcvar_num(cvar_iSemiclipEnemies, 1)
        set_task(13.0, "ChangeSemiclipEnemies", TASK_SEMICLIP) // TASK_SEMICLIP should be defined like #define TASK_SEMICLIP 1996 ; or to any other number that hasn't been used yet to define tasks with.  
...
public ChangeSemiclipEnemies(TASKID) {
    set_pcvar_num(cvar_iSemiclipEnemies, 0)
}  
...
public LoadSpawns()
{
	...
	//if (!LibraryExists("zp50_core", LibType_Library)) // ! mark is post edit!!
	//{
	//	Label_FailState:
	//	plugin_pause()
...
public MessageTeamInfo(msg_id, msg_dest)
{
	...
	switch (szTeam[0])
	{
		case 'T': g_iTeam[id] = _:zp_get_team(id)//_:CS_TEAM_T
		case 'C': g_iTeam[id] = _:zp_get_team(id)//_:CS_TEAM_CT
		case 'S':
		{
