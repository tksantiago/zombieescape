...
public plugin_init()
{
	...
	set_task(0.6, "showhud", 0, _, _, "b");
}

public showhud(id)
{
	//set_dhudmessage(255, 255, 255, -1.0, 0.00, 0, 0.5, 2.0, 0.08, 3.0)
	//show_dhudmessage(id,"Zombi ( %i ) Insan^n[%i]   TUR   [%i]", (g_scorezombies+g_scorehumans+1), g_scorezombies, g_scorehumans)

	set_dhudmessage(4, 255, 240, -1.0, 0.00, 0, 0.5, 2.0, 0.08, 3.0)
	show_dhudmessage(id,"zombie   VS   Humanos")
	set_dhudmessage(255, 255, 255, -1.0, 0.00, 0, 0.5, 2.0, 0.08, 3.0)
	show_dhudmessage(id,"^n (%i)          (%i)", g_scorezombies, g_scorehumans)
}
...
public logevent_round_end()
{
	...
	if (!fnGetZombies())
	{
		...
		show_dhudmessage(0, "=====FFUGA BAŞARILI! ====^n^n^nEscape Success!")
	...
	else if (!fnGetHumans())
	{
		...
		show_dhudmessage(0, "===== FUGA SEM SUCESSO! ====^n^n^nEscape Failed!")
...
public showStatus(id)
{
	if(!is_user_bot(id) && is_user_connected(id))
	{
		...
		if (team1 == team2)
		{
			...
			ShowSyncHudMsg(id, g_status_sync, "%s - %d HP / %d AP / %d CP", name, get_user_health(pid), get_user_armor(pid), g_ammopacks[pid])
		}
		else
		{
			set_hudmessage(color1, 50, color2, -1.0, 0.60, 1, 0.01, 3.0, 0.01, 0.01, -1)
...
show_menu_game(id)
{
	...
	// 5. Help
	len += formatex(menu[len], charsmax(menu) - len, "\r6.\w %L^n^n", id, "MENU_BICAK")
			...
			if (get_pcvar_num(cvar_extraitems))
			{
				...
				if (g_isalive[id])
				{
					if (g_buycountround[id] < get_pcvar_num(cvar_maxbuyround))
						...
						zp_colored_print(id, "^x04[ZE]^x01 Limite maximo de compra atingido")
				}
				else
					zp_colored_print(id, "^x04[ZE]^x01 %L", id, "CMD_NOT")
			}
...
public menu_zclass(id, menuid, item)
{
	...
	{
		zp_colored_print(id, "^x04[ZE]^x01 Seu nível e insuficiente para este zumbi.")
	}
...
{
	// Mostrar mod info 
	//zp_colored_print(0, "^x03[Zombie Escape Mod By Multipower]")
	//zp_colored_print(0, "^x04[%L]", LANG_PLAYER, "NOTICE_INFO1")
}
			...
			if (g_zombie[id])
			{
				...
				else // Zombies
				{
					...
					else
					{
						...
						format(clawmodel, charsmax(clawmodel), "models/starzombie/%s", clawmodel)
...
public native_register_zombie_class(const name[], const info[], const model[], const clawmodel[], hp, speed, Float:gravity, Float:knockback, level)
{
	...
	for (i = 0; i < size; i++)
	{
		...
		// Precache clawmodel
		formatex(prec_mdl, charsmax(prec_mdl), "models/starzombie/%s", buffer)
	...
	if (ArrayGetCell(g_zclass_new, g_zclass_i))
	{
		...
		formatex(prec_mdl, charsmax(prec_mdl), "models/starzombie/%s", clawmodel)
		engfunc(EngFunc_PrecacheModel, prec_mdl)
	}
